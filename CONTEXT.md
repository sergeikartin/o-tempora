...
**Lane**:
One of the three top-level content tracks — People, Wars & Conflicts, Events & Inventions — each with its own fetch/score/tag/output path (`packages/data-pipeline/docs/adr/0001-wars-discoveries-people-separate-lanes.md`). `fetch`'s per-lane CLI selector (`--lane=<value>`) uses `people`/`wars`/`discoveries` as its values — `discoveries` is this pipeline's canonical public name for the Events & Inventions lane (`packages/data-pipeline/docs/adr/0012-lane-scoped-fetch.md`). Fetch-stage code and the curated source file predate that naming and still say "events" (`events-curated.raw.json`, `fetchEventsEnrichment`) — a known inconsistency, tracked separately (`.scratch/discoveries-file-rename/spec.md`), not resolved by this rename.
_Avoid_: track, events (as the lane name outside fetch's pre-existing internal file/function names)
