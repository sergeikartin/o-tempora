...
2. The timeline shows two lanes: **People** (horizontal bars from birth to death year) and **Events & Inventions** (single points on their date).
...
- Two lanes: People (range bars) and Events & Inventions (point markers).
...
- Per-person fields: name, birth year, death year, occupation tag(s), region tag(s), fame score (sitelink count), short description, Wikipedia URL.
- Per-event fields: name, date (or start/end for short ranges), occupation/category tag, region tag, fame score, short description, Wikipedia URL.
...
- People lane and Events/Inventions lane, both hardcoded from Wikidata.
