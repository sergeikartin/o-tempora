...
|---|---|---|
| People dataset (name, birth/death year, occupation tag, region tag, fame score, description, Wikipedia URL) | `packages/shared-types/src/data/people.json`, imported by `packages/web` and bundled into the production build | Generated by the pipeline's Output stage; never hand-edited, never written to at runtime |
| Events dataset (name, date, occupation/category tag, region tag, fame score, description, Wikipedia URL) | `packages/shared-types/src/data/events.json`, imported by `packages/web` and bundled into the production build | Same as above |
