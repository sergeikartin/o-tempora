...
This describes the technical architecture for the v1 scope defined in `project-overview.md`. The project has two parts: a **frontend app** (`packages/web`) — a read-only, client-side, continuously zoomable timeline of world history built on vis-timeline — and a **data pipeline** (`packages/data-pipeline`) that curates the frontend's dataset from Wikidata. They ship independently: the pipeline runs offline to produce static JSON, and the frontend only ever reads that JSON. Both are npm-workspace packages under `packages/`, alongside `packages/shared-types` (see Monorepo tooling in the Stack table).
...
| Monorepo tooling | npm workspaces | Roots the repo at a workspace `package.json` (`workspaces: ["packages/*"]`) containing three packages — `packages/web` (frontend), `packages/data-pipeline`, and `packages/shared-types` — so `packages/shared-types`, holding the `Person`/`HistoricalEvent`/`Category`/`Region` type contracts plus the generated `people.json`/`events.json` datasets under `packages/shared-types/src/data/`, can be imported by both `packages/data-pipeline` and (from Unit 3/4) `packages/web` without either project importing the other. Added in Unit 2 to resolve the package-boundary question `code-standards.md`'s "share those types" rule left open; data-pipeline's own pipeline tests use Node's built-in `node:test` runner rather than adding Vitest there, since Vitest is only declared for the frontend above. Common devDependencies used by more than one package (`typescript`, `@types/node`) are declared once in the root `package.json` and apply workspace-wide; a dependency used by only one package (e.g. `tsx` in `packages/data-pipeline`) is declared in that package's own `package.json`, not hoisted |
| Lint / boundary tooling | ESLint + typescript-eslint + Steiger + `@feature-sliced/steiger-plugin` | ESLint (flat config, `js.configs.recommended` + `tseslint.configs.strict` + React Hooks/Refresh rules) lints `packages/web`'s TS/TSX; Steiger with the `@feature-sliced/steiger-plugin` recommended ruleset enforces mini-FSD layer boundaries (`shared → features → widgets → app`, no upward/sideways imports) against `packages/web/src`. Added in Unit 3 alongside the `packages/web` scaffold |
...
| Layer | Owns |
|---|---|
| `packages/web/src/shared/` | Business-agnostic logic and constants: date conversion (`lib/dates.ts`), zoom/viewport constants (`config/`), the `Person`/`HistoricalEvent` type contracts (`types/`, re-exported from `packages/shared-types`), generic UI primitives (`ui/`). The generated `people.json`/`events.json` datasets are not duplicated here — the frontend imports them directly from `packages/shared-types/src/data/` (see System Boundaries and Storage Model) |
| `packages/web/src/features/` | Independent user-facing behaviors with their own state: fame-tier selection, occupation filter, region filter, entity selection, and the combined filter-intersection logic that decides what's visible |
| `packages/web/src/widgets/` | Composite UI blocks that combine `shared/types` data with `features/` behavior: the `timeline-canvas` (vis-timeline wrapper, owns pan/zoom bounds and group config), `filter-bar`, and `detail-panel` |
| `packages/web/src/app/` | Entry point, global providers, global styles; composes the three widgets directly into the full-screen layout |

### Data Pipeline (`packages/data-pipeline`)

| Folder | Owns |
|---|---|
| `packages/data-pipeline/fetch/` | SPARQL queries against the Wikidata Query Service; writes raw, unmodified query results to `packages/data-pipeline/data/raw/`. Never computes scores or tags — fetching and transforming are kept separate so a bad transform doesn't require re-querying Wikidata |
| `packages/data-pipeline/transform/` | Computes fame score from sitelink counts, maps raw Wikidata occupation/location claims onto the app's fixed occupation and region tag sets |
| `packages/data-pipeline/output/` | Writes the final `people.json` and `events.json` into `packages/shared-types/src/data/` |
| `packages/data-pipeline/data/raw/` | Checked-in snapshots of raw Wikidata query results, for reproducibility and diffing between pipeline runs |
...
1. **Fetch** — SPARQL queries pull candidate humans (with birth/death year, occupation claims, sitelink count) and candidate events (inventions, wars, discoveries, with date and sitelink count). A third query fetches reign/term-of-office periods (Wikidata P39 "position held," qualified with P580/P582 start/end dates) for the candidate people pool, parameterized on the person Q-IDs the first query already found — the one query in Fetch that depends on another query's output rather than scanning the corpus independently, since "which people are candidates" has to exist first. Results are written as-is to `packages/data-pipeline/data/raw/` and checked into version control, so a pipeline run is reproducible and reviewable in a diff without re-hitting Wikidata.
...
|---|---|---|
| People dataset (name, birth/death year, occupation tag, region tag, fame score, description, Wikipedia URL, optional reign/term-of-office periods) | `packages/shared-types/src/data/people.json`, imported by `packages/web` and bundled into the production build | Generated by the pipeline's Output stage; never hand-edited, never written to at runtime |
| Events dataset (name, date, occupation/category tag, region tag, fame score, description, Wikipedia URL, optional end date and parent-conflict name) | `packages/shared-types/src/data/events.json`, imported by `packages/web` and bundled into the production build | Same as above |
| Raw Wikidata query results | `packages/data-pipeline/data/raw/`, checked into version control | Untouched SPARQL output; re-running Fetch overwrites these, giving a diffable record of what changed between pipeline runs |
