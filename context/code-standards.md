...
- There is no database and no backend service. `shared/data/*.json` is the only data the running app reads.
- `shared/data/*.json` is always machine-generated by the pipeline's Output stage — never hand-edited. There is no override mechanism in v1; a correction means fixing the pipeline's Score or Tag logic and regenerating.
...
- `shared/types/` — the `Person` and `HistoricalEvent` interfaces (data contracts, not a full entities layer).
- `shared/data/` — generated JSON datasets (`people.json`, `events.json`); read-only, never hand-edited.
...
- `data-pipeline/transform/` — scoring and tagging logic.
- `data-pipeline/output/` — final JSON generation and copy-to-frontend step.
