...
- **The People and Events lanes are two separate, synced `Timeline` instances, not one `Timeline` with two groups.** A single vis-timeline instance has exactly one global vertical scroll position shared across all its groups, so it can't give each lane an independent fixed-height scroll region. The two instances are kept on the same visible time window via a `rangechange` listener on each that calls `setWindow({animation: false})` on the other, guarded by a shared `isSyncing` flag against feedback (calling `setWindow` on the target re-fires *its own* `rangechange`, which must not bounce back to the source). The People lane is 2x the height of the Events lane (`flex: 2` / `flex: 1` in `TimelineCanvas.module.css`) — this ratio is a CSS layout concern, not a vis-timeline option. Only the Events lane (the bottom one) renders the shared time axis (`orientation: { axis: 'none' }` on the People instance) — showing it twice would be redundant.
- **Wheel-zoom is disabled (`zoomable: false`) in favor of dedicated +/- buttons** that call `Timeline.zoomIn()`/`zoomOut()` directly (unaffected by `zoomable: false`, which only gates the library's own wheel handler). The freed-up wheel gesture is repurposed for scrolling: `verticalScroll: true` makes a plain vertical wheel scroll a lane's stacked content, and `horizontalScroll: true` makes a horizontal wheel gesture (or drag, which is a separate always-on interaction) pan through time instead. Both options are set identically on both lane instances.
