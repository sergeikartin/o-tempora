...
- Strict mode is required throughout the project, in both `packages/web` and `packages/data-pipeline`.
...
- Use `interface` for object contracts (e.g. `Person`, `HistoricalEvent`), and share those types between `packages/web` and `packages/data-pipeline` rather than redefining them in both places.
...
- **The People, Wars & Conflicts, and Events & Inventions lanes are three separate, synced `Timeline` instances, not one `Timeline` with three groups.** A single vis-timeline instance has exactly one global vertical scroll position shared across all its groups, so it can't give each lane an independent fixed-height scroll region. The three instances are kept on the same visible time window via a `rangechange` listener on each that calls `setWindow({animation: false})` on the *other two*, guarded by a single shared `isSyncing` flag against feedback (calling `setWindow` on a target re-fires *its own* `rangechange`, which must not bounce back to the source or cross-bounce between the other two). The lanes render top to bottom in that order, sized `flex: 2` / `flex: 1` / `flex: 1` in `TimelineCanvas.module.css` — People keeps its dominant share, the other two split the remainder evenly (an arbitrary starting ratio, easy to retune, absent specific product guidance). Only the bottom lane (Events & Inventions) renders the shared time axis (`orientation: { axis: 'none' }` on the other two) — showing it twice would be redundant.
- **`HistoricalEvent.category` doubles as the undeclared lane split between Wars & Conflicts and Events & Inventions.** `category === "invention"` goes to Events & Inventions; everything else (`"war"`, `"politics"`) goes to Wars & Conflicts. Within the Wars & Conflicts lane, an entry with `endDate` set renders as a `range` bar (only ever true for actual wars — see `HistoricalEvent.endDate`'s own doc comment in `packages/shared-types`); everything else in that lane renders as a `point`. A zero-width range (`date === endDate`, a real Wikidata edge case, e.g. the Six-Day War) is widened by one year for rendering only, via the same helper used for a person's missing `deathYear`.
- **A person's `reignPeriods` render as an overlay inside their own lifespan bar, not a second bar.** Each reign period becomes its own `range` item sharing a vis-timeline `subgroup` (the person's id) with that person's own lifespan item — see `map-to-items.ts`. Getting vis-timeline to actually *overlap* same-subgroup items (rather than stack them into separate rows) needs the *`DataGroup`'s* `subgroupStack` config to set an internal `doInnerStack` flag, not the similarly-named `TimelineOptions.stackSubgroups` (a common trap — that option only gates whether the group-level config is honored, and does nothing on its own; confirmed by reading vis-timeline's actual stacking source, not assumed from the option name). `options.ts`'s `PEOPLE_GROUPS` sets `subgroupStack` to an object with one dummy key that's never a real person id, purely to flip `doInnerStack` on globally for the People group without opting any real person's subgroup into separated (non-overlapping) stacking — every real person subgroup's own `subgroupStack` entry is implicitly absent/falsy, which is what produces overlap.
- **A `DataItem.title` string is rendered as vis-timeline's own JS-driven hover popup (`showTooltips: true`, the default), not a static HTML `title` attribute.** Don't look for it in the initial DOM — it only appears in the DOM on actual mouse hover. Used for `partOfWarName` in the Wars & Conflicts lane and for reign-period date ranges in the People lane.
- **Wheel-zoom is disabled (`zoomable: false`) in favor of dedicated +/- buttons** that call `Timeline.zoomIn()`/`zoomOut()` directly (unaffected by `zoomable: false`, which only gates the library's own wheel handler). The freed-up wheel gesture is repurposed for scrolling: `verticalScroll: true` makes a plain vertical wheel scroll a lane's stacked content, and `horizontalScroll: true` makes a horizontal wheel gesture (or drag, which is a separate always-on interaction) pan through time instead. Both options are set identically on all three lane instances (the zoom buttons themselves still only call `zoomIn()`/`zoomOut()` on the Events & Inventions instance — sync propagates the resulting window to the other two, same mechanism as panning/dragging).

## Data and Storage

- There is no database and no backend service. `packages/shared-types/src/data/*.json` (imported directly, not copied into `packages/web`) is the only data the running app reads.
...
- The Fetch stage never merges, scores, or tags data — it only writes raw Wikidata query results to `packages/data-pipeline/data/raw/`, untouched.
...
Frontend (`packages/web/src/`), organized by mini-FSD layer, lowest to highest (`shared → features → widgets → app`):
...
- `shared/types/` — re-exports the `Person`, `HistoricalEvent`, `Category`, and `Region` types from `packages/shared-types` (the canonical definitions, shared with `packages/data-pipeline` via an npm workspace — see `architecture.md`'s Stack table and System Boundaries section). Not a full entities layer.
- No `shared/data/` folder: the generated `people.json`/`events.json` datasets live in `packages/shared-types/src/data/` and are imported directly from there — not copied into `packages/web`, to avoid keeping two copies in sync.
...
Data pipeline (`packages/data-pipeline/`), unaffected by mini-FSD — it's a separate, non-UI project:

- `packages/data-pipeline/fetch/` — raw Wikidata queries only.
  - `queries/historical-events.ts` — exports `WAR_TYPE_QID` (`"Q198"`), the one `?type` class that gets range-bar treatment; the canonical source other stages import it from, rather than each redefining the literal.
  - `queries/reigns.ts`, `fetch-reigns.ts` — reign/term-of-office periods (Wikidata P39 "position held," qualified with P580/P582 start/end dates), parameterized on a batch of candidate person Q-IDs rather than scanning the corpus independently. Reads `people.raw.json` back off disk to get those Q-IDs, so it must run after `fetch-people.ts`; writes `people-reigns.raw.json`. Broad by design — any qualified P39 statement with a start date counts, not just literal monarch titles, since Wikidata has no single closed class for "ruler." Batched (50 Q-IDs per request) with per-batch failure tolerance — a failed batch just means those people get no reign data, not a fatal pipeline error.
- `packages/data-pipeline/transform/` — grouping, scoring, and tagging logic:
  ...
  - `group-reigns.ts` — a separate grouping shape from `group-rows.ts` (one *list* of periods per person, not one collapsed record), since `people-reigns.raw.json` is genuinely denormalized-on-purpose, not a per-entity collapse target.
  ...
  - `score.ts` — sorts by fame score (sitelink count) descending and slices to a fame-tier ceiling, passed explicitly by the caller: `PEOPLE_FAME_TIER_CEILING` (3000) and `EVENTS_FAME_TIER_CEILING` (1000) — two different constants, not one shared value, since the candidate pools are very different sizes.
  - `index.ts` — orchestrates group → tag → score for both the people and events lanes; also exports `loadReignsMap()`, read by Output.
- `packages/data-pipeline/output/` — final JSON generation:
  - `write-datasets.ts` — constructs the final `Person`/`HistoricalEvent` shape from transform's output, applies the drop rules (missing name, Wikipedia article, description, or required date), and reports what was dropped and why. Also gates `HistoricalEvent.endDate` to `WAR_TYPE_QID` entries only (a battle/treaty that happens to carry a Wikidata end-time claim still renders as a point, not a bar) and attaches `Person.reignPeriods` by looking up each person's id in the reigns map — extra map entries for people who don't make the final cut are simply never read.
  - `index.ts` — runs the full transform → output flow (`npm run build-data`) and writes `people.json`/`events.json` directly into `packages/shared-types/src/data/` — a single copy, not a `packages/data-pipeline/output/` copy plus a duplicate under `packages/web/src/shared/data/`.
- `packages/shared-types/` — the canonical `Person`/`HistoricalEvent`/`Category`/`Region` type contracts, imported by `packages/data-pipeline` (and, from Unit 3/4, by `packages/web/src/shared/types/`) via an npm workspace. `packages/shared-types/src/data/` holds the pipeline's generated `people.json`/`events.json`, imported directly by `packages/web` (no `packages/web/src/shared/data/` copy). Not itself part of either the frontend's mini-FSD layering or the pipeline's stage structure — see `architecture.md`'s Stack table and System Boundaries section.
