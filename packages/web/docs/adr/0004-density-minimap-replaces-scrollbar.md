...
- The Minimap's domain is the app's actual pannable range (`PAN_MIN_YEAR`, `packages/web/src/shared/config/viewport.ts`) to present, not the full historical data extent some records reach. Extending `PAN_MIN_YEAR` itself is a separate, unmade decision.
