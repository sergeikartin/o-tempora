...
# Mountain Profile density minimap replaces the scrollbar
...
**Literal minimap thumbnail** (miniature individual items, like a code editor's minimap). Rejected: expensive to render at ~4,000 items across the full range, and the app is pure SVG/D3 today with no canvas element — this would force adding one just for this feature.
...
- Row Depth for the profile is a distinct computation from `assignRows`' live per-render call — same packing logic, different (fixed) scale input and full-domain scope instead of viewport-scoped.
- The profile's domain is the app's actual pannable range (`PAN_MIN_DATE = -2750`) to present, not the full historical data extent some records reach (People to -3345, Milestones to -3499). Extending `PAN_MIN_DATE` itself is a separate, unmade decision.
