...
Milestones-only, keyed on `MilestoneCategory`. The taxonomy stays all 20 values for filtering, tooltips, and filter-chip labels — only *color* folds it down to 3 legible groups, each one a darker, richer step of the People-domain hue its content actually echoes (same pigment, different register: People reads as the lit, vivid layer; Conflicts/Milestones as the darker ink marking what happened). A prior 20-hue-per-category palette sat at ~11° hue separation — below any CVD-safe spacing, and more colors than a legend can hold anyway. This set, together with the flat Conflict Color above, is small enough (4 colors) to validate *every* pair against every other, not just neighbors — and unlike the Occupation Domain palette above, it clears the full CVD target even muted (spreading the 3 groups' lightness further apart bought back what the lower chroma cost): lightness band, chroma floor, and CVD separation (worst all-pairs ΔE 8.1, protan) all pass; contrast is a WARN only for Society & Governance (2.8:1), mitigated the same way as the domain palette above.

| Group | Hex | Rhymes with (People domain) | `MilestoneCategory` values |
|---|---|---|---|
| Knowledge & Culture | `#4B4597` | Humanities | `science-theory`, `philosophy-education`, `culture-arts`, `religion-mythology`, `archaeology-anthropology` |
| Technology & Industry | `#008456` | Science & Technology | `medicine-health`, `communication`, `transportation`, `infrastructure`, `everyday-technology`, `energy-industry`, `exploration`, `architecture-design`, `environment-geology` |
| Society & Governance | `#BC8118` | Public Figure | `food-agriculture`, `society-administration`, `commerce-finance`, `social-movements`, `sports-entertainment`, `law-jurisprudence` |
