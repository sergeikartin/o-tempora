...
test('motionDurationMs reads a ms-suffixed custom property from :root', () => {
  document.documentElement.style.setProperty('--motion-duration-test-ms', '250ms');
...
test('motionDurationMs reads an s-suffixed custom property as milliseconds', () => {
  document.documentElement.style.setProperty('--motion-duration-test-s', '0.3s');
...
test('motionDurationMs caches the first read, ignoring a later value change', () => {
  document.documentElement.style.setProperty('--motion-duration-test-cache', '100ms');
  expect(motionDurationMs('--motion-duration-test-cache')).toBe(100);

  document.documentElement.style.setProperty('--motion-duration-test-cache', '900ms');
  expect(motionDurationMs('--motion-duration-test-cache')).toBe(100);
});
