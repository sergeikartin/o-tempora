...
import { CONFLICTS_MILESTONES_FILTER_VALUES, CONFLICTS_MILESTONES_FILTER_LABELS } from '../../../shared/config';
...
test('renders one toggle pill per filter value (Conflicts + the 2 Milestone Category Groups), none pressed by default', () => {
  ...
  for (const value of CONFLICTS_MILESTONES_FILTER_VALUES) {
    const button = getByLabelText(`Filter by ${CONFLICTS_MILESTONES_FILTER_LABELS[value]}`) as HTMLButtonElement;
...
test('marks selected values as pressed', () => {
  const { getByLabelText } = render(
    <ConflictsMilestonesFilters selectedValues={['conflicts']} onToggleValue={vi.fn()} />,
  );
