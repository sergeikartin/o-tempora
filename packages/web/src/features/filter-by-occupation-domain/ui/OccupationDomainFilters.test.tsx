...
test('renders one toggle pill per OccupationDomain, none pressed by default', () => {
  ...
  for (const domain of OCCUPATION_DOMAINS) {
    const button = getByLabelText(
      `Filter by ${DOMAIN_LABELS[domain]}`,
...
test('marks selected domains as pressed', () => {
  ...
  expect(
    getByLabelText('Filter by Science & Technology').getAttribute(
      'aria-pressed',
    ),
  ).toBe('true');
  expect(getByLabelText('Filter by Arts').getAttribute('aria-pressed')).toBe(
    'false',
  );
});
