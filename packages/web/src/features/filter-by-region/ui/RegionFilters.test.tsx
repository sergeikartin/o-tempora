...
test('renders one toggle pill per Region, none pressed by default', () => {
  const { getByLabelText } = render(
    <RegionFilters selectedRegions={[]} onToggleRegion={vi.fn()} />,
  );

  expect(
    getByLabelText('Filter by Western Europe').getAttribute('aria-pressed'),
  ).toBe('false');
  expect(
    getByLabelText('Filter by Western Asia').getAttribute('aria-pressed'),
  ).toBe('false');
});
...
test('marks selected regions as pressed', () => {
  ...
  expect(
    getByLabelText('Filter by Northern Africa').getAttribute('aria-pressed'),
  ).toBe('false');
});
...
test('clicking a pill calls onToggleRegion with that region', () => {
  ...
  const { getByLabelText } = render(
    <RegionFilters selectedRegions={[]} onToggleRegion={onToggleRegion} />,
  );
