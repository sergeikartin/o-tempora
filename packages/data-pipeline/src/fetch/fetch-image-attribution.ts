...
// Reads the two raw files fetch-descriptions.ts/fetch-events-enrichment.ts
// already wrote (each now carrying a raw P18 `image` URI, see this ticket's
// query changes) and, for every entity that resolved an image, runs a
// second batched Commons `imageinfo` pass to resolve `imageAttribution`
...
// batched-sparql-fetch.ts. Ordered after both in fetch/index.ts, since it
// depends on their output being on disk.
export async function fetchImageAttribution(): Promise<void> {
  ...
  const peopleAttribution = await batchedCommonsImageAttributionFetch(peopleEntries);

  console.log(`Fetching Commons attribution for ${discoveryEntries.length} discovery images...`);
  const discoveriesAttribution = await batchedCommonsImageAttributionFetch(discoveryEntries);
  ...
  console.log(
    `Wrote ${peopleAttribution.size} people + ${discoveriesAttribution.size} discovery attributions to ${outputPath}`,
  );
}
