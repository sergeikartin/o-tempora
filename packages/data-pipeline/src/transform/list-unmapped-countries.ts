...
// Dumps any country Q-ID a Wars/Discoveries event carries (across both
...
//
// Both Wars and Discoveries are the hand-curated + enriched list now, not a
...
// transformWars/transformDiscoveries).
const { wars } = validateEnrichedWarsFile(JSON.parse(fs.readFileSync("data/raw/wars-curated-enriched.raw.json", "utf8")));
...
for (const countryId of [...wars.flatMap((war) => war.countries), ...discoveries.flatMap((event) => event.countries)]) {
