...
4. **Output** — each lane's `buildPeople`/`buildConflicts`/`buildMilestones` (`write-datasets.ts`) runs twice, once per language (`Lang = 'en' | 'ru'`), against the identical transformed rows: row inclusion is decided once, off English fields only, so both language files always share the same entity set; per-field text (`name`/`tagline`/`description`) resolves to the Russian value when building the Russian file and it's present, English otherwise (`resolveField`/`resolveOptionalField` — see `CONTEXT.md`'s **Field Fallback**). Each entity's permanent `row` (CONTEXT.md's **Row Depth**) is computed once per lane off the English build (`row-assignment.ts`'s `assignPersonRows`/`assignConflictsMilestonesRows` — People among themselves, Conflicts and Milestones together) and applied to both language builds by id, since row assignment is language-independent (`docs/adr/0005-row-assignment-moves-to-the-pipeline.md`). Each built lane array is then split by `splitByDetailLevel` into 4 Detail Level delta files (CONTEXT.md's **Detail Level**, `docs/adr/0006-detail-level-merges-data-depth-and-payload-tier.md`) at `DETAIL_LEVEL_FAME_SCORE_FLOORS` (`packages/shared-types`) — each delta holds only the entities newly added versus the previous level, not a self-contained cumulative file. Writes four files per lane per language (`people.detail1.json` .. `people.detail4.json`, `people.detail1.ru.json` .. `people.detail4.ru.json`, etc.) to pipeline-owned `data/output/` (gitignored); `npm run publish-data` copies them into `packages/shared-types`, keeping "compute" and "publish" as separate steps.
