...
`VITE_SENTRY_DSN` (`packages/web/.env.example`) is the only monitoring config. Unset — the default in local dev — `src/shared/lib/init-monitoring.ts`'s `initMonitoring()` is a no-op, and `logError()` (`src/shared/lib/log-error.ts`) still logs to the console. Set, `main.tsx` initializes Sentry with `sampleRate: 1` and `tracesSampleRate: 1` (capture every error and every pageload transaction) before the app renders. `browserTracingIntegration()` is what turns those pageload transactions into Sentry's Performance/Insights view, including Web Vitals (LCP/CLS/INP/TTFB) with no separate instrumentation. Both sample rates are at 100% since traffic is low enough not to meaningfully touch Sentry's free-tier quotas (the span quota backing traces is smaller than the error quota — worth revisiting if traffic grows). One DSN, shared across both the `en` and `ru` builds — they're one JS bundle (`docs/deployment.md`), not split per language. In CI, the deploy workflow reads it from the `VITE_SENTRY_DSN` repo secret and passes it to the build step, since Vite bakes `VITE_*` env vars in at build time.
