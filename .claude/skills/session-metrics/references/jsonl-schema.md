...
| `cache_creation_input_tokens`                 | **Yes** — cache_write rate         | Tokens written into the cache. **Sum** of the 5m and 1h ephemeral buckets. Used as a 5m-tier fallback for legacy transcripts lacking the nested object. | **tracked** (CacheWr column). |
| `cache_creation.ephemeral_5m_input_tokens`    | **Yes** — 1.25× input (5m rate)    | Portion of the cache write landing in the 5-minute TTL tier.                                         | **tracked** — priced at the 5m rate (Proposal A, shipped) |
| `cache_creation.ephemeral_1h_input_tokens`    | **Yes** — 2× input (1h rate)       | Portion of the cache write landing in the 1-hour TTL tier.                                           | **tracked** — priced at the 1h rate (Proposal A, shipped) |
...
| `speed`                                       | Metadata (drives multiplier)       | `"standard"` or `"fast"` (Claude Code `/fast` mode).                                                 | **tracked** (Mode column). Fast-mode premium multiplier **is** applied in cost math since v1.64.0 (per-model factor; suppress with `--no-fast-premium`) — see `references/pricing.md`. |
