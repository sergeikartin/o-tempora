...
- **`packages/web` will break once the alt-data-sources pipeline changes are published.** `App.tsx` imports `@same-sky/shared-types/src/data/events.json` directly, and `map-to-items.ts` reads `person.category` and client-side-filters `event.category` to split one `events.json` into Wars/Discoveries lanes — both break once `publish-data` produces `wars.json`/`discoveries.json` instead (ticket 18 of `.scratch/alt-data-sources/`) and once People's `Person.category`/`occupationTags` become `occupationDomain: OccupationDomain` (ticket 17). Deliberately deferred — the user chose to implement the pipeline side first and update `packages/web` in a separate follow-up unit, not bundled into either pipeline ticket. Don't run `publish-data` for real until that frontend follow-up is scheduled, or the app won't build.
