...
- Finish the Fame Tier Redesign unit: re-run `npm run fetch --workspace packages/data-pipeline` cleanly (retry or after `git checkout` of the partial `data/raw/*.json` files), then `npm run build-data --workspace packages/data-pipeline` followed by `npm run publish-data --workspace packages/data-pipeline`, then spot-check `people.json`/`wars.json`/`discoveries.json` have nothing below 30 sitelinks per the spec's Verification Checklist.
...
- **`packages/web` no longer typechecks against `packages/shared-types` as of the alt-data-sources pipeline changes (confirmed via `tsc -b`, not just reasoning).** `App.tsx` imports `@same-sky/shared-types/src/data/events.json` directly (gone — split into `wars.json`/`discoveries.json`, ticket 18); `map-to-items.ts`/tests reference the now-removed `HistoricalEvent` type, `event.date`/`endDate` (now `War.startYear`/`endYear`, `Discovery` has neither), and `person.category`/`Person.birthYear`/`deathYear` (now `Person.startYear`/`endYear`, ticket 23; `occupationDomain` still pending, ticket 17). `packages/web/CLAUDE-decisions.md`/`CLAUDE-patterns.md` have superseded-but-not-yet-updated-in-code notes marking exactly which decisions this affects. Deliberately deferred — the user chose to implement the pipeline side first and update `packages/web` in a separate follow-up unit, not bundled into any pipeline ticket. Don't run `publish-data` for real (i.e. actually build `packages/web`) until that frontend follow-up is scheduled.
