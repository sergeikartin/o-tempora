...
- [ ] `fetchImageAttribution` accepts a lane parameter (`"people" | "wars" | "discoveries"` or all three) and only processes the requested lane(s)' entities.
- [ ] Each lane writes its own raw file (`<lane>-image-attribution.raw.json`), not a shared combined file.
- [ ] Existing per-lane Commons `imageinfo` batching/pacing behavior (`batched-commons-image-attribution-fetch.ts`) is unchanged — only the output shape and lane-scoping change.
