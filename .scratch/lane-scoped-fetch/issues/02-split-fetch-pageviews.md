...
- [ ] `fetchPageviews` accepts a lane parameter (`"wars" | "discoveries"` or both) and only processes the requested lane(s)' entities.
- [ ] Each lane writes its own raw file (`<lane>-pageviews.raw.json`), not a shared combined file.
- [ ] Existing per-lane pacing behavior (`batched-pageviews-fetch.ts`) is unchanged — only the output shape and lane-scoping change.
- [ ] Test coverage: a lane-scoped call writes only that lane's file and leaves the other's raw file untouched.
