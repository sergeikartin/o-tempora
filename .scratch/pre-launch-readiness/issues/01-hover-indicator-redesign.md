...
The vertical guide-line feature (hovering the Mountain Profile draws a line spanning all lanes/axes at that year — `MountainProfile.tsx`, `TimelineCanvas.tsx`, `TimelineCanvas.module.css`) was built (commit `45730f0`) and reverted the same day (commit `b1e370b`), with no rationale recorded in either commit message.
...
Resolve:
- What specifically was wrong with the original implementation (visual clutter against the mountain profile / lane density, performance, interaction feel, something else)?
