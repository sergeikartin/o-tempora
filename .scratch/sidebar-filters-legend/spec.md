...
- `PersonItem`, `ConflictItem`, and `MilestoneItem` (in `map-to-items.ts`) each gain a `regionTags: Region[]` field. For Conflicts/Milestones this is a direct pass-through of the existing `Region[]` data. For People, `mapPeople` translates the existing `UnRegion[]` `regionTags` into `Region[]` at mapping time using a new, complete lookup table (below) — so the generic filter function never needs to know two enums exist.
- New lookup table `UN_REGION_TO_REGION: Record<UnRegion, Region>` (home: `shared/config`, alongside `viewport.ts`/`occupation-domain-colors.ts`), a **complete** mapping — every one of the 22 `UnRegion` values resolves to exactly one of the 6 `Region` values, so no person is ever excluded from every region pill just because their sub-region doesn't fit neatly. Built from the measured People distribution (Aug 2026 dataset, 3,770 people) so every bucket's real population is known:
...
- New pure function `filterByRegion<T extends { regionTags: Region[] }>(items: T[], selectedRegions: Region[]): T[]`, added alongside `filterByFameScore`/`filterByOccupationDomain` in `map-to-items.ts`. Empty `selectedRegions` returns `items` unchanged. Non-empty: keeps items with at least one tag in the selected set (OR). An item with an empty `regionTags` array never matches a non-empty selection, so it drops out whenever any region filter is active — this includes Conflicts/Milestones whose curated `regionTags` is genuinely `[]` (e.g. World War II).
