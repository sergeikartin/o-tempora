...
- All four tickets implemented and resolved (2026-08-12) — see each ticket's `## Answer`. Mid-implementation correction on ticket 04: filtering happens on the raw per-lane datasets in `TimelineCanvas.tsx`, before mapping to render shapes, so `filterByRegion` took a `regionsOf` accessor rather than the originally-specced `regionTags` field on `PersonItem`/`ConflictItem`/`MilestoneItem` — `spec.md` updated to match. Full regression (typecheck, lint, boundaries, 196 tests, build) and a live manual smoke test of all three filters combined (AND across categories) confirmed green.
