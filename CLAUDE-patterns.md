...
- `Temporal.PlainDate` for all dates in app and pipeline code — never construct a legacy `Date` except at the one documented adapter (see Timeline Rendering below).
...
- `HistoricalEvent.category === "invention"` routes to Events & Inventions; everything else to Wars & Conflicts (range bar if `endDate` is set, else a point).
